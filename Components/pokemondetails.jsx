...
export default function PokemonDetails() {
  ...
  const [movesDetails, setMovesDetails] = useState({}); // moveName => details
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto p-6">
      {/* Back Button */}
      <motion.button whileHover={{ x: -6 }} onClick={() => navigate(-1)} className="flex items-center text-blue-600 hover:text-blue-800 mb-8 font-semibold" aria-label="Back to Pokédex">
      ...
      <div className={`rounded-2xl p-6 mb-10 shadow-lg bg-gradient-to-br ${typeColors[primaryType]}`}>
        <div className="flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="flex-1 text-white">
            ...
            <p className="font-semibold text-xl mb-3 drop-shadow-md">#{pokemon.id.toString().padStart(3, "0")}</p>

            <div className="flex gap-3 mb-5">
              {pokemon.types.map(({ type }) => (
                <span key={type.name} className="px-4 py-1 rounded-full font-semibold bg-black bg-opacity-30 capitalize">{type.name}</span>
              ))}
            </div>

            <div className="flex gap-6 font-semibold text-lg mb-6 drop-shadow-sm">
              <div><p>Height</p><p>{(pokemon.height / 10).toFixed(1)} m</p></div>
              <div><p>Weight</p><p>{(pokemon.weight / 10).toFixed(1)} kg</p></div>
            </div>

            <button onClick={playSound} className="inline-flex items-center gap-2 rounded-lg bg-white bg-opacity-90 hover:bg-opacity-100 px-5 py-2 font-semibold text-blue-700 shadow-lg transition" aria-label="Play Pokémon Cry">
      ...
      <Tab.Group>
        <Tab.List className="flex space-x-2 bg-blue-50 p-1 rounded-xl shadow-inner">
          {["About", "Stats", "Evolution", "Moves", "Media"].map(tab => (
            <Tab key={tab} className={({ selected }) => `w-full py-3 font-semibold rounded-lg transition-colors duration-300 ${selected ? "bg-white shadow text-blue-700" : "text-blue-600 hover:text-blue-800"}`}>
              {tab}
            </Tab>
          ))}
        </Tab.List>
        ...
        <Tab.Panels className="mt-6 text-gray-700">
          {/* About Tab - render species info */}
          <Tab.Panel className="focus:outline-none bg-white rounded-xl p-6 shadow max-h-[450px] overflow-auto space-y-3">
            {species ? (
              <>
                <h3 className="text-lg font-semibold mb-4">About</h3>
                <p className="italic whitespace-pre-line max-h-32 overflow-y-auto">{species.flavor_text_entries.find(e => e.language.name === "en")?.flavor_text.replace(/\f/g, " ") || "No description."}</p>
          ...
          <Tab.Panel className="focus:outline-none bg-white rounded-xl p-6 shadow max-h-[450px] overflow-auto">
            ...
            {pokemon.stats.map(stat => (
              <div key={stat.stat.name} className="flex items-center gap-4 mb-3">
                ...
                <progress className="flex-1 rounded h-6" max="255" value={stat.base_stat} aria-label={`${stat.stat.name} stat`}>{stat.base_stat}</progress>
          ...
          {/* Moves Tab (simple move names without detail fetching) */}
          <Tab.Panel className="focus:outline-none bg-white rounded-xl p-6 shadow max-h-[450px] overflow-auto">
          ...
          <Tab.Panel className="focus:outline-none bg-white rounded-xl p-6 shadow space-y-8 min-h-[300px] overflow-auto max-h-[600px] text-gray-700">
            <section>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {gallery.length > 0 ? (
                  gallery.map(media => (
                    <motion.div key={media.id} whileHover={{ scale: 1.05 }} className="rounded-lg overflow-hidden border border-gray-300 cursor-pointer" onClick={() => setSelectedImage(media)} title={media.title}>
                      <img src={media.url} alt={media.title} className="w-full h-28 object-contain bg-gray-100" loading="lazy" onError={e => { e.target.onerror = null; e.target.src = "/placeholder-image.png"; }} />
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {videos.length ? videos.map(video => (
                  <motion.div key={video.id} className="rounded-lg overflow-hidden shadow-lg cursor-pointer" whileHover={{ scale: 1.05, boxShadow: "0 8px 20px rgba(59,130,246,0.4)" }} onClick={() => setPlayingVideo(video.id)}>
                    {playingVideo === video.id ? (
                      <iframe src={`${video.url}?autoplay=1`} title={video.title} className="w-full h-56 rounded-lg" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
                    ) : (
      ...
      {/* Image Modal */}
      {selectedImage && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-6" onClick={() => setSelectedImage(null)} aria-modal="true" role="dialog">
          <motion.div className="relative max-w-5xl w-full bg-white rounded-lg overflow-hidden shadow-lg" onClick={e => e.stopPropagation()} initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }}>
            <button onClick={() => setSelectedImage(null)} className="absolute top-3 right-3 text-gray-600 hover:text-gray-900 transition" aria-label="Close Image">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
            <img src={selectedImage.url} alt={selectedImage.title} className="w-full max-h-[75vh] object-contain bg-gray-100" loading="lazy" />
